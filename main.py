...
if __name__ == '__main__':
    ...
    print(' '.join(['java', '-jar', 'lib/evosuite-1.1.0.jar', '-class', class_name, '-projectCP', 'java_classpath']))

    subprocess.run(['javac', java_file, '-d', 'java_classpath', '-cp', ':'.join(case_specific_libs)])
    subprocess.run(['kotlinc', kotlin_file, '-jvm-target', '1.8', '-d', 'kotlin_classpath', '-cp', ':'.join(case_specific_libs)])
