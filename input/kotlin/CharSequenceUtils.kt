...
//        }
    ...
    fun indexOf(cs: CharSequence?, searchChar: Int, start: Int): Int {
        if (cs == null) throw NullPointerException()
        var start = start
        if (cs is String) {
    ...
    fun regionMatches(
        ...
        if (cs == null || substring == null) throw NullPointerException()
     ...
     *
     * @param source the `CharSequence` to be processed.
     * @return the resulting char array, never null.
     * @since 3.11
     */
    @JvmStatic
    fun toCharArray(source: CharSequence): CharArray {
        val len: Int = StringUtils.length(source)
        if (len == 0) {
...
fun main() {
    ...
    val stringBuffer1 = stringBuffer0.insert(0, 'y')
    val int0 = CharSequenceUtils.lastIndexOf(stringBuffer1, stringBuffer1, 0)
    print(int0)
}